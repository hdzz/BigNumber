 ...
 *    > Description:
 *			Plus two BigNumbers
 *    > Created Time: 2016-01-15 12:11:13
**/
...
{//Plus two BigNumbers
    ...
    if(A.dot>B.dot){
        ...
        if(!B.dot) {dot=first_free-val;alloc.construct(first_free++,A.val[len_a++]);}
              ...
              int dis=B.dot-A.dot;
              while(dis--) alloc.construct(first_free++,B.val[len_b++]);
              if(!A.dot) {dot=first_free-val;alloc.construct(first_free++,B.val[len_b++]);}
          }
    ...
    while(len_a!=(A.len-1)||len_b!=(B.len-1)){
        ...
        if((*first_free-'0')>=10){
            (*first_free)-=10;
            first_free++;
            alloc.construct(first_free,'1');
        }else {first_free++;alloc.construct(first_free,'0');}
    }
    if(*first_free=='1') {alloc.construct(++first_free,'+');}
    else *first_free='+';
